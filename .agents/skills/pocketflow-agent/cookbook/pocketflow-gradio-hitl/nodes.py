...
class DecideActionResponse(BaseModel):
    ...
    action: Literal[
        "check-weather", "book-hotel", "follow-up", "result-notification"
    ] = Field(description="The next action to take.")
    reason: str = Field(description="Why you chose this action.")
    question: Optional[str] = Field(
        None,
        description="Guidance or follow-up question to the user. Required if action is 'follow-up'.",
    )
    city: Optional[str] = Field(
        None,
        description="The city to check the weather. Required if action is 'check-weather'.",
    )
    date: Optional[str] = Field(
        None,
        description="The date to check the weather. Optional if action is 'check-weather'.",
    )
    hotel: Optional[str] = Field(
        None,
        description="The name of the hotel to be booked. Required if action is 'book-hotel'.",
    )
    checkin_date: Optional[str] = Field(
        None, description="The check-in date. Required if action is 'book-hotel'."
    )
    checkout_date: Optional[str] = Field(
        None, description="The check-out date. Required if action is 'book-hotel'."
    )
    result: Optional[str] = Field(
        None,
        description="The result notification message. Required if action is 'result-notification'.",
    )
...
"""
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if action == "check-weather":
            ...
            flow_log.put(f"➔   Agent decided to check weather for: {data['city']}")
        elif action == "book-hotel":
            session["book_hotel_params"] = {
                "hotel": data["hotel"],
                "checkin_date": data["checkin_date"],
                "checkout_date": data["checkout_date"],
            }
            flow_log.put(f"➔   Agent decided to book hotel: {data['hotel']}")
        elif action == "follow-up":
            session["follow_up_params"] = {"question": data["question"]}
            flow_log.put(f"➔   Agent decided to follow up: {data['question']}")
        elif action == "result-notification":
            session["result_notification_params"] = {"result": data["result"]}
            flow_log.put(f"➔   Agent decided to notify the result: {data['result']}")
...
class CheckWeather(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        flow_log: Queue = shared["flow_queue"]
        flow_log.put(f"➔   Check weather result: {exec_res}")
...
class BookHotel(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        flow_log: Queue = shared["flow_queue"]
        flow_log.put(f"➔   Book hotel result: {exec_res}")
