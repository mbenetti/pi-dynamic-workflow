...
class SQLSchema(BaseModel):
    sql: str = Field(description="The SQLite SQL query")
...
class GenerateSQL(StructuredNode):
    def __init__(self):
        from utils.call_llm import get_instructor_client

        super().__init__(
            response_model=SQLSchema, client=get_instructor_client(), model="gpt-4o"
        )
...
Please generate the correct SQLite SQL query for this question.
"""
        return prompt

    def post(self, shared, prep_res, exec_res):
        sql_query = exec_res.sql.strip().rstrip(";")
...
class ExecuteSQL(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if success:
            ...
            if isinstance(result_or_error, list):
                if column_names:
                    ...
                    print(
                        "-"
                        * (
                            sum(len(str(c)) for c in column_names)
                            + 3 * (len(column_names) - 1)
                        )
                    )
            ...
            max_attempts = shared.get(
                "max_debug_attempts", 3
            )  # Get max attempts from shared
            ...
            if shared["debug_attempts"] >= max_attempts:
                ...
                shared["final_error"] = (
                    f"Failed to execute SQL after {max_attempts} attempts. Last error: {shared['execution_error']}"
                )
                return
            else:
...
class DebugSQL(StructuredNode):
    ...
    def prep(self, shared):
        ...
        failed_sql = shared.get("generated_sql")
        error_message = shared.get("execution_error")
        prompt = f"""
...
"""
    ...
    def post(self, shared, prep_res, exec_res):
        corrected_sql = exec_res.sql.strip().rstrip(";")
        shared["generated_sql"] = corrected_sql  # Overwrite with the new attempt
        shared.pop(
            "execution_error", None
        )  # Clear the previous error for the next ExecuteSQL attempt
